...
root_path = r'/home/data3/liuyansong/Lysinear/Datasets/SYSU-CD'
...
def read_images(root):
    ...
    imgs_Av = glob.glob(root + '/val/time1/*.png')
    imgs_Bv = glob.glob(root + '/val/time2/*.png')
    labelsv = glob.glob(root + '/val/label/*.png')
    imgs_Att = glob.glob(root + '/test/time1/*.png')
    imgs_Btt = glob.glob(root + '/test/time2/*.png')
    labelstt = glob.glob(root + '/test/label/*.png')
    return imgs_At, imgs_Bt, labelst, imgs_Av, imgs_Bv, labelsv, imgs_Att, imgs_Btt, labelstt
...
class SYSUCDDataset(torch.utils.data.Dataset):
    def __init__(self, mode='train'):
        ...
        self.train_dataset_before, self.train_dataset_after, self.train_dataset_change, self.val_dataset_before, self.val_dataset_after, self.val_dataset_change, self.test_dataset_before, self.test_dataset_after, self.test_dataset_change = read_images(root_path)
        if mode == 'train':
            print('训练集加载了' + str(len(self.train_dataset_before)) + '张图片')
        elif mode == 'val':
            print('验证集加载了' + str(len(self.val_dataset_before)) + '张图片')
        elif mode == 'test':
    ...
    def __getitem__(self, item):
        if self.mode == 'train':
            ...
            before = Image.open(self.val_dataset_before[item]).convert('RGB')
            after = Image.open(self.val_dataset_after[item]).convert('RGB')
            change = Image.open(self.val_dataset_change[item]).convert('L')
    ...
    def __len__(self):
        if self.mode == 'train':
            ...
            return len(self.val_dataset_before)
        elif self.mode == 'test':
