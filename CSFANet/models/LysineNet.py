...
from models.basic_modules import Channel_Attention, Align
...
class MY_NET(nn.Module):
    def __init__(self,num_classes=2):
        ...
        # x_layer1:[2, 64, 64, 64] x_layer2:[2, 128, 32, 32] x_layer3:[2, 256, 16, 16] x_layer4:[2, 512, 8, 8]
        # self.layer1_pos = nn.Parameter(torch.randn(64, 64))
        # self.layer2_pos = nn.Parameter(torch.randn(32, 32))
        # self.layer3_pos = nn.Parameter(torch.randn(16, 16))
        # self.layer4_pos = nn.Parameter(torch.randn(8, 8))
        # self.pos_embedding = [self.layer1_pos, self.layer2_pos, self.layer3_pos, self.layer4_pos]
        ...
        self.align1 = Align(in_channel=960, out_channel=32)
        self.align2 = Align(in_channel=1152, out_channel=64)
        self.align3 = Align(in_channel=2304, out_channel=128)
        self.align4 = Align(in_channel=7680, out_channel=256)
    ...
    def forward(self,x1,x2):
        h, w = x1.shape[2:]
        ## x_layer1:[2, 64, 64, 64] x_layer2:[2, 128, 32, 32] x_layer3:[2, 256, 16, 16] x_layer4:[2, 512, 8, 8]
        x1_layer1, x1_layer2, x1_layer3, x1_layer4 = self.backbone(x1)
        x2_layer1, x2_layer2, x2_layer3, x2_layer4 = self.backbone(x2)

        # align = Align(channels=[16, 24, 32, 96, 320])
        x1_layer1_sampled, x2_layer1_sampled = self.align1([x1_layer1, x1_layer2, x1_layer3, x1_layer4], [x2_layer1, x2_layer2, x2_layer3, x2_layer4], 0) # [4, 32, 64, 64]
        x1_layer2_sampled, x2_layer2_sampled = self.align2([x1_layer1, x1_layer2, x1_layer3, x1_layer4], [x2_layer1, x2_layer2, x2_layer3, x2_layer4], 1) # [4, 64, 32, 32]
        x1_layer3_sampled, x2_layer3_sampled = self.align3([x1_layer1, x1_layer2, x1_layer3, x1_layer4], [x2_layer1, x2_layer2, x2_layer3, x2_layer4], 2) # [4, 128, 16, 16]
        x1_layer4_sampled, x2_layer4_sampled = self.align4([x1_layer1, x1_layer2, x1_layer3, x1_layer4], [x2_layer1, x2_layer2, x2_layer3, x2_layer4], 3) # [4, 256, 8, 8]

        # x1_layer1_sampled, x1_layer2_sampled, x1_layer3_sampled, x1_layer4_sampled = self.swa(x1_layer1_sampled, x1_layer2_sampled, x1_layer3_sampled, x1_layer4_sampled)
        # x2_layer1_sampled, x2_layer2_sampled, x2_layer3_sampled, x2_layer4_sampled = self.swa(x2_layer1_sampled, x2_layer2_sampled, x2_layer3_sampled, x2_layer4_sampled)

        inter1_a, inter1_b = self.cross1(x1_layer1_sampled, x2_layer1_sampled)  # [4, 32, 64, 64]
        inter2_a, inter2_b = self.cross2(x1_layer2_sampled, x2_layer2_sampled)  # [4, 64, 32, 32]
        inter3_a, inter3_b = self.cross3(x1_layer3_sampled, x2_layer3_sampled)  # [4, 128, 16, 16]
        inter4_a, inter4_b = self.cross4(x1_layer4_sampled, x2_layer4_sampled)  # [4, 256, 8, 8]
        ...
        out = torch.cat([F.interpolate(add3, add1.shape[2:], mode='bilinear', align_corners=True),
                         F.interpolate(add2, add1.shape[2:], mode='bilinear', align_corners=True), add1], dim=1)  #[4,224,64,64]
        ...
        output1 = F.interpolate(aux_1,size=(h, w), mode='bilinear', align_corners=True)
        output2 = F.interpolate(aux_2,size=(h, w), mode='bilinear', align_corners=True)
        output3 = F.interpolate(aux_3, size=(h, w), mode='bilinear', align_corners=True)
